...
import {
  listIndividualQuestionFilesForReview,
  getFileReview,
  updateReviewStatus,
  getFileViewUrl,
  type Review,
...
export default function IndividualQuestionReviewPage() {
  ...
  const loadReviews = async () => {
    setLoading(true);
    try {
      const data = await listIndividualQuestionFilesForReview();
      setReviews(data);
    } catch (e) {
      console.error("Failed to load individual question reviews", e);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-6 h-full min-h-0">
        <div className="flex flex-col min-h-0">
          <div className="flex items-center justify-between pb-2">
            <h1 className="text-2xl font-bold">개별 문항 검토</h1>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-sm text-gray-500 text-center py-8">검토할 개별 문항 파일이 없습니다.</div>
