...
export default function ProjectManagementPage() {
  ...
  const [form, setForm] = useState<CreateProjectRequest & { owner_department?: string }>({
    ...
    owner_department: "",
    target_individual_items_count: 20, // 기본값 20
  });
  ...
  return (
    <div className="w-full min-h-[calc(100vh-64px)] px-4 md:px-6 py-4 text-gray-900 space-y-4">
      <section className="rounded-3xl border border-slate-200/60 bg-white/80 p-5 shadow-[0_18px_45px_-28px_rgba(15,23,42,0.55)] backdrop-blur flex items-center justify-between">
          ...
          <div className="mt-2 text-xs text-slate-500">
            {isAdmin ? (
              ...
              <span>
                ...
                <span className="text-gray-700_toggle_scrollbar text-slate-700">
                  {myDepartments.map(deptLabel).join(", ") || "-"}
                </span>
              </span>
            ) : (
              <span className="text-rose-600">권한: 없음 (프로젝트 생성/삭제 불가)</span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={[
              "h-10 rounded-2xl border px-3 text-sm font-semibold inline-flex items-center gap-2 transition",
              selectedIds.size === 0 || (!isAdmin && !isLead)
                ? "border-slate-200 text-slate-300 bg-white"
                : "border-rose-200 text-rose-700 bg-white hover:bg-rose-50/60",
            ].join(" ")}
          >
          ...
          <button
            ...
            className={[
              "h-10 rounded-2xl border px-4 text-sm font-semibold text-white shadow-[0_14px_34px_-22px_rgba(15,11,152,0.85)] transition",
              canCreateAny
                ? "border-indigo-600 bg-indigo-700 hover:bg-indigo-800"
                : "border-slate-300 bg-slate-300 shadow-none",
            ].join(" ")}
          >
            프로젝트 등록
          </button>
      ...
      {listLoading ? (
        <div className="rounded-3xl border border-slate-200/60 bg-white/80 p-5 shadow-[0_18px_45px_-28px_rgba(15,23,42,0.55)] backdrop-blur text-sm text-slate-600">
          프로젝트 목록을 불러오는 중입니다...
        </div>
      ) : (
        <ProjectListTable
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">개별 문항 목표 개수</label>
                <input
                  type="number"
                  name="target_individual_items_count"
                  value={(form as any).target_individual_items_count ?? 20}
                  onChange={handleChange}
                  min="1"
                  className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
                  placeholder="20"
                />
                <p className="mt-1 text-xs text-gray-500">기본값: 20개</p>
              </div>
